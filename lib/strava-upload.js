...
const _token_expired = async (accessTokenData) => {
...
module.exports = async (accessTokenData) => {
  // Get an authenticated strava instance
  let strava = new stravaAPI.client(accessTokenData.access_token);
  ...
  for (const file of tcxfiles){
    ...
    if (await _token_expired(accessTokenData)){
      console.log('*** refreshing access token')
      accessTokenData = await stravaAPI.oauth.refreshToken(accessTokenData.refresh_token);
      strava = new stravaAPI.client(accessTokenData.access_token);
    }
