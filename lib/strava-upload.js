...
module.exports = async (strava) => {
  ...
  for (const file of tcxfiles){
    ...
    // Do upload
    console.log(`processing file ${Object.keys(filesProcessed).length + 1} of ${tcxfiles.length} : ${files.getFullPath(file.name)}`)
    try {
      const payload = await strava.uploads.post({
        ...
        statusCallback: (err, payload) => {
          if (filesProcessed[file.name]) {
            ...
            inFlight--;
            filesProcessed[file.name] = 'failure'
            ...
            inFlight--;
            filesProcessed[file.name] = 'success'
    ...
    if (Object.keys(filesProcessed).length >= 5) break;
  }
}
